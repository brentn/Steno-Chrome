...
LookupTranslator.prototype.initialize = function(){
  ...
  var self=this;
  this._formatter = new NullFormatter();
  try {
    chrome.storage.sync.get({UNDO_SIZE:20, TRANSLATOR_TYPE:'FULL'}, function(items) {
      self._history = new History(items.UNDO_SIZE);
      switch (items.TRANSLATOR_TYPE) {
        case 'SIMPLE': self._formatter = new SimpleFormatter(); break;
        case 'FULL': self._formatter = new FullFormatter(); break;
      }
    });
  } catch(ex) {}
